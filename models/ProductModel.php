...
class ProductModel {
    ...
    public function createProduct($name, $description) {
        ...
        if ($stmt->execute()) {
            return $this->conn->insert_id; // Trả về ID tự tăng vừa chèn
        }
        return false;
    }

    /**
     * Thêm một liên kết nền tảng (Tiki/Shopee) cho sản phẩm
     */
    public function addPlatformLink($productId, $platformName, $url) {
    // 1. Kiểm tra xem đã tồn tại link của sàn này cho sản phẩm này chưa
    ...
    if ($result->num_rows > 0) {
        // 2. Nếu ĐÃ CÓ: Cập nhật lại URL mới (nếu cần) và giữ nguyên 1 dòng duy nhất
        ...
        // 3. Nếu CHƯA CÓ: Mới tiến hành chèn dòng mới
